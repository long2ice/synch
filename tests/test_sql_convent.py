...
class TestSqlConvent(TestCase):
    def test_add_char(self):
        ...
        self.assertEqual(ret, "alter table test.test add column name String")
    ...
    def test_add_int(self):
        ...
        ret = SqlConvert.to_clickhouse("test", sql)
        self.assertEqual(ret, "alter table test.test add column name Int32")

    def test_add_decimal(self):
        sql = "alter table test add name decimal(10,2) not null after id"
        ret = SqlConvert.to_clickhouse("test", sql)
        self.assertEqual(ret, "alter table test.test add column name Decimal(10,2)")

    def test_add_datetime(self):
        sql = "alter table test add name datetime not null after id"
        ret = SqlConvert.to_clickhouse("test", sql)
        self.assertEqual(ret, "alter table test.test add column name DateTime")
